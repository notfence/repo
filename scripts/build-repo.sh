...
for manifest in "${manifest_files[@]}"; do
  ...
  release_updated_at="$(jq -er '.updated_at' "${release_json}")"
  printf '%s\t%s\t%s\t%s\t%s\n' \
    "${expected_package}" "${source_repository}" "${release_tag}" "${asset_id}" "${release_updated_at}" \
    >> "${state_entries}"
  ...
  dpkg-deb -f "${deb_path}" >> "${packages_file}"
  ...
  jq -n \
    ...
    --arg releaseDate "${release_published_at}" \
